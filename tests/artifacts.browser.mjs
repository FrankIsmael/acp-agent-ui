...
try {
  ...
  await wait(() => evaluate(`document.querySelector('textarea')?.placeholder === 'Continue the conversation…'`), 'connected chat');
  ...
  await click('button[aria-label="Close artifacts"]');
  await sleep(500);
  ...
  await wait(() => evaluate(`!!document.querySelector('button[aria-label="Send"]')`), 'finished stream');
  ...
  await evaluate(`[...document.querySelectorAll('button')].find(b=>b.textContent==='Code')?.click()`);
  const edited = app.replace('A little space to create.', 'Edited locally.');
  await fill('textarea[aria-label="Edit Creative counter"]', edited);
  ...
  await evaluate(`[...document.querySelectorAll('button')].find(b=>b.textContent==='Code')?.click()`);
  assert.ok((await evaluate(`document.querySelector('textarea[aria-label="Edit Creative counter"]').value`)).includes('Edited locally.'));
  await click('button[aria-label="Close artifacts"]');
  ...
  await click('button[aria-label="Download file"]');
  ...
  await click('button[aria-label="Share file"]');
  await wait(() => evaluate(`document.body.innerText.includes('File downloaded for sharing.')`), 'share download fallback');
  ...
  await click('button[aria-label="Copy content"]');
  await wait(() => evaluate(`document.body.innerText.includes('Copied')`), 'copy feedback');
  ...
  await wait(() => evaluate(`document.querySelector('textarea[aria-label="Edit Python script"]')?.value === 'print("hello")'`), 'editable Python source');
  ...
  await fill('textarea[aria-label="Edit Python script"]', 'print("still editable")');
  await wait(() => evaluate(`document.body.innerText.includes('Could not save locally.')`), 'storage quota feedback');
  assert.equal(await evaluate(`document.querySelector('textarea[aria-label="Edit Python script"]').value`), 'print("still editable")');

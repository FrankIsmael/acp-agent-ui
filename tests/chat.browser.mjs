...
try {
  ...
  await cdp('Emulation.setLocaleOverride', { locale: 'es-MX' });
  const englishHtml = await (await fetch('http://127.0.0.1:5198/settings', { headers: { 'Accept-Language': 'es-MX' } })).text();
  assert.ok(englishHtml.includes('lang="en"') && englishHtml.includes('New conversation'), 'SSR defaults to English regardless of browser language');
  const spanishHtml = await (await fetch('http://127.0.0.1:5198/settings', { headers: { Cookie: 'locale=es' } })).text();
  assert.ok(spanishHtml.includes('lang="es"') && spanishHtml.includes('Nueva conversación'), 'SSR respects saved Spanish preference');
  await cdp('Page.navigate', { url: 'http://127.0.0.1:5198/settings' });
  await wait(() => evaluate(`document.querySelector('h1')?.textContent === 'Settings' && Object.keys(document.querySelector('button[lang="es"]') ?? {}).some(key => key.startsWith('__reactProps'))`), 'English settings hydration');
  await click('button[lang="es"]');
  await wait(() => evaluate(`document.documentElement.lang === 'es' && document.querySelector('h1')?.textContent === 'Ajustes' && document.querySelector('a[href="/artifacts"]')?.textContent.includes('Artefactos')`), 'switch to Spanish across the app');
  assert.ok(await evaluate(`document.cookie.includes('locale=es')`), 'language preference persisted');
  await cdp('Page.reload');
  await wait(() => evaluate(`document.documentElement.lang === 'es' && document.querySelector('button[lang="es"]')?.getAttribute('aria-pressed') === 'true'`), 'Spanish survives reload');
  await click('a[href="/artifacts"]');
  await wait(() => evaluate(`document.querySelector('h1')?.textContent === 'Artefactos'`), 'Spanish survives navigation');
  await click('a[href="/settings"]');
  await wait(() => evaluate(`!!document.querySelector('button[lang="en"]')`), 'return to settings');
  await click('button[lang="en"]');
  await wait(() => evaluate(`document.documentElement.lang === 'en' && document.querySelector('h1')?.textContent === 'Settings'`), 'switch back to English');
  await cdp('Page.reload');
  await wait(() => evaluate(`document.documentElement.lang === 'en' && document.querySelector('button[lang="en"]')?.getAttribute('aria-pressed') === 'true'`), 'English survives reload');
  ...
  const area = `document.querySelector('[aria-label="Messages"]')`;
  ...
  await click('button[aria-label="Stop"]');
  await wait(() => evaluate(`!!document.querySelector('button[aria-label="Send"]')`), 'cancel confirmed');
  ...
  await click('button[aria-label="Send"]');
  await wait(() => evaluate(`!!document.querySelector('button[aria-label="Stop"]')`), 'new turn after stop');
  await cdp('Page.reload');
  await wait(() => evaluate(`!!document.querySelector('button[aria-label="Stop"]')`), 'reload restores busy state');
  ...
  await click('button[aria-label="Stop"]');
  await wait(() => evaluate(`!!document.querySelector('button[aria-label="Send"]')`), 'stop after reload');
  ...
  await wait(() => evaluate(`document.querySelector('h1')?.textContent === 'History' && document.body.innerText.includes('A remembered conversation')`), 'history page loads saved sessions');
  ...
  await wait(() => evaluate(`document.querySelector('[aria-label="Messages"]')?.textContent.includes('An answer from yesterday')`), 'saved chat replay');
  assert.equal(await evaluate(`document.querySelector('[aria-label="Messages"]').textContent.split('An answer from yesterday').length - 1`), 1, 'replay appears once');
  await cdp('Page.reload');
  await wait(() => evaluate(`document.querySelector('[aria-label="Messages"]')?.textContent.includes('A question from yesterday')`), 'saved chat survives reload');
  await click('a[href="/skills"]');
  await wait(() => evaluate(`document.querySelector('h1')?.textContent === 'Skills' && document.body.innerText.includes('Project conventions')`), 'skills list replaces placeholder');
  ...
  await wait(() => evaluate(`!!document.querySelector('button[aria-label="Model"]')`), 'cached draft model selector');
  await evaluate(`document.querySelector('button[aria-label="Model"]').focus()`);
  ...
  await wait(() => evaluate(`document.querySelector('button[aria-label="Model"]')?.textContent.includes('Model Two')`), 'draft model saved');
  await cdp('Page.reload');
  await wait(() => evaluate(`document.querySelector('button[aria-label="Model"]')?.textContent.includes('Model Two')`), 'draft model survives reload');
  assert.equal(newSessions, sessionsBeforeDraft, 'choosing model does not allocate a session');
  assert.deepEqual(errors, [], 'no uncaught browser errors');
  console.log('PASS: English default, bilingual SSR, language switching and persistence, streaming follow, manual scroll, desktop/mobile containment, ACP cancellation, partial reply, next turn, reload stop, endpoint validation, saved history and chats, replay and reload, skills, draft model preference.');
} catch (error) {
